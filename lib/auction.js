...
export function parseListingHtml(html) {
  ...
  $("td.tbl_auction_right").each((_, priceCell) => {
    ...
    const appraisalRaw = parseWon(prices[0]);
    const soldPriceRaw = parseWon(prices[2]);
    ...
    // statusText에 "(100%) (110%)"처럼 퍼센트가 두 개(최저가 비율, 낙찰가율) 나오는
    // 경우가 있어 첫 번째 값만 집으면 틀린다. 감정가/낙찰가로 직접 계산해 정확한
    // 낙찰가율을 구하고, 계산이 불가능할 때만 마지막 퍼센트 값으로 대체한다.
    const rateMatches = [...statusText.matchAll(/\((\d+)%\)/g)];
    const rateMatch = rateMatches[rateMatches.length - 1];
    const computedRate =
      appraisalRaw && soldPriceRaw
        ? String(Math.round((Number(soldPriceRaw) / Number(appraisalRaw)) * 100))
        : "";
    ...
    items.push({
      ...
      rate: computedRate || (rateMatch ? rateMatch[1] : ""),
...
// "망월동 1148 하남미사롯데캐슬헤븐시티2 7층751호" → "망월동"
export function dongOf(address) {
  const m = String(address).match(/^(\S+?[동읍면리])\s/);
  return m ? m[1] : "";
}
...
// "2024-68878" → "2024타경68878" (사건번호로 검색하는 사람이 많음)
function caseLabel(caseNo) {
  return String(caseNo).replace(/^(\d{4})-(\d+)$/, "$1타경$2");
}

// 글 맨 앞에 들어가는 요약 문단. items는 { usage, address, rate }만 있으면 됩니다.
// (scripts/enrich-posts.js가 기존 글을 보강할 때도 이 함수를 씁니다)
export function summarizeAuction(items, dateStr) {
  const [y, m, d] = dateStr.split("-").map(Number);
  const usages = countBy(items, (it) => it.usage);
  const dongs = countBy(items, (it) => dongOf(it.address));

  const sentences = [
    `${y}년 ${m}월 ${d}일 기준 최근 일주일간 경기도 하남시 법원경매에서 총 ${items.length}건이 낙찰되었습니다.`,
  ];

  let line2 = `용도별로는 ${usages.map(([k, n]) => `${k} ${n}건`).join(", ")}`;
  if (dongs.length) line2 += `이며, 지역별로는 ${dongs.map(([k, n]) => `${k} ${n}건`).join(", ")}`;
  sentences.push(line2 + "입니다.");

  const rated = items
    .filter((it) => Number(it.rate) > 0)
    .map((it) => ({ address: it.address, r: Number(it.rate) }));
  if (rated.length) {
    const avg = Math.round(rated.reduce((a, x) => a + x.r, 0) / rated.length);
    const max = rated.reduce((a, x) => (x.r > a.r ? x : a));
    const min = rated.reduce((a, x) => (x.r < a.r ? x : a));
    let s;
    if (rated.length === 1) {
      s = `감정가 대비 낙찰가율은 ${avg}%였습니다.`;
    } else if (max.r === min.r) {
      s = `감정가 대비 낙찰가율은 모든 물건이 ${max.r}%였습니다.`;
    } else {
      s = `감정가 대비 낙찰가율은 평균 ${avg}%이며, 가장 높은 물건은 ${max.r}%(${max.address}), 가장 낮은 물건은 ${min.r}%(${min.address})였습니다.`;
    }
    sentences.push(s);
  }

  sentences.push("물건별 감정가·최저가·낙찰가와 권리관계는 아래와 같습니다.");
  return sentences.join(" ");
}

// 제목 뒤에 붙는 검색용 문구: "망월동·신장동 오피스텔 4건 낙찰"
export function auctionTitleSuffix(items) {
  const dongs = countBy(items, (it) => dongOf(it.address))
    .slice(0, 3)
    .map(([k]) => k);
  const usages = countBy(items, (it) => it.usage);
  const usageText = usages.length > 1 ? `${usages[0][0]} 등` : usages[0] ? usages[0][0] : "";
  return [dongs.join("·"), usageText, `${items.length}건 낙찰`].filter(Boolean).join(" ");
}

export function buildMarkdown(items, dateStr) {
  ...
  const sections = [summarizeAuction(items, dateStr)];
  for (const [usage, list] of groups) {
    const entries = list.map((it) => {
      const lines = [`### ${it.address} (${usage})`];
      const courtText = /법원|지원/.test(it.court) ? ` (${it.court})` : "";
      lines.push(`- 사건번호: ${caseLabel(it.caseNo)}${courtText}`);
      const areaParts = [];
  ...
  sections.push(
    "---\n\n이번 주 하남시 법원경매 낙찰 결과였습니다. 관심 있는 물건이나 입찰 상담이 필요하시면 왕가부동산(010-3366-7187)으로 편하게 문의해 주세요."
  );
...
export async function collectWeeklyAuctionPost() {
  ...
  const title = `하남시 최신 법원경매 낙찰 현황 (${dateTo.replace(/-/g, ".")} 기준) - ${auctionTitleSuffix(items)}`;
  const content = buildMarkdown(items, dateTo);
